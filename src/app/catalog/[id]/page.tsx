...
export default function ProductPage() {
   ...
   const [categories, setCategories] = useState<Category[]>([])
    ...
    const displayImages = product.images?.length > 0 
        ? product.images 
        : (product.main_image_url ? [{ id: 'placeholder', image_url: product.main_image_url, sort_order: 0 }] : [])
    ...
    return (
        <>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div>
                        ...
                        <div className="flex flex-wrap items-center gap-3 mb-4">
                            <Link href={`/masters/${product.master_id}`} className="flex items-center gap-2 hover:opacity-80 group">
                                ...
                                <span className="text-sm text-gray-600 group-hover:text-firm-orange transition">{product.master_name}</span>
                            </Link>
                            <span className="text-gray-300">|</span>
                            <div className="flex items-center gap-1">
                                <span className="text-yellow-400">★</span>
                                <span className="font-semibold text-sm">{product.rating || 'Нет оценок'}</span>
                                <span className="text-gray-400 text-sm">({product.reviews_count || 0} отзывов)</span>
                            </div>
                        </div>
                            ...
                            {product.color && (
                                <div className="bg-gray-50 rounded-xl p-3">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <div className="w-4 h-4 rounded-full border border-gray-300" style={{ backgroundColor: product.color.toLowerCase() }} />
                        ...
                        <div className="border-t border-gray-200 pt-4">
                            ...
                            <div className="py-2">
                                    ...
                                    <div className="space-y-3 w-full">
                                        {product.category && (
                                            <div className="flex py-1 justify-between items-start">
                                                <span className="text-gray-500 text-sm w-2/5 md:w-1/3">Категория:</span>
                                                <span className="font-medium text-sm text-right w-3/5 md:w-2/3">{product.category}</span>
                                            </div>
                                        )}
                                        {product.technique && (
                                            <div className="flex py-1 justify-between items-start">
                                                <span className="text-gray-500 text-sm w-2/5 md:w-1/3">Техника вязания:</span>
                                                <span className="font-medium text-sm text-right w-3/5 md:w-2/3">{product.technique}</span>
                                            </div>
                                        )}
                                        {product.size && product.size !== 'Не применимо' && (
                                            <div className="flex py-1 justify-between items-start">
                                                <span className="text-gray-500 text-sm w-2/5 md:w-1/3">Размер:</span>
                                                <span className="font-medium text-sm text-right w-3/5 md:w-2/3">{product.size}</span>
                                            </div>
                                        )}
                                        {product.color && (
                                            <div className="flex py-1 justify-between items-start">
                                                <span className="text-gray-500 text-sm w-2/5 md:w-1/3">Цвет:</span>
                                                <div className="font-medium text-sm text-right w-3/5 md:w-2/3 flex justify-end items-center gap-2">
                                                    <span className="inline-block w-3 h-3 rounded-full" style={{ backgroundColor: product.color.toLowerCase() }} />
                                        ...
                                        <div className="flex py-1 justify-between items-start">
                                            <span className="text-gray-500 text-sm w-2/5 md:w-1/3">Просмотры:</span>
                                            <span className="font-medium text-sm text-right w-3/5 md:w-2/3">{product.views}</span>
                                        </div>
                                        <div className="flex py-1 justify-between items-start">
                                            <span className="text-gray-500 text-sm w-2/5 md:w-1/3">Добавлен:</span>
                                            <span className="font-medium text-sm text-right w-3/5 md:w-2/3">
                                ...
                                {activeTab === 'care' && product.care_instructions && (
                                    <p className="text-gray-700 text-sm leading-relaxed">{product.care_instructions}</p>
                                )}
                                    ...
                                    <div>
                                        ...
                                        {product.reviews && product.reviews.length > 0 ? (
                                                    ...
                                                    <div key={review.id} className="border-b border-gray-100 pb-4">
                                                        <div className="flex items-center gap-3 mb-2">
                                                            ...
                                                            <div>
                                                                ...
                                                                <div className="flex items-center gap-1">
                                                                    {[...Array(5)].map((_, i) => (
                                                                        <span key={i} className={i < review.rating ? 'text-yellow-400 text-xs' : 'text-gray-300 text-xs'}>★</span>
                                                                    ))}
                                            ...
                                            <div className="text-center py-8 bg-gray-50 rounded-lg">
                                                ...
                                                {session && session.user?.role !== 'master' && !isAuthor && (
                                                    <button onClick={() => setShowReviewModal(true)} className="mt-2 text-firm-orange hover:underline text-sm">
                                                        Будьте первым
                                                    </button>
                                                )}
                                            </div>
            ...
            <AnimatePresence>
                {showReviewModal && (
                    <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setShowReviewModal(false)}>
                        <div className="bg-white rounded-2xl max-w-md w-full p-6" onClick={(e) => e.stopPropagation()}>
                            <h3 className="font-['Montserrat_Alternates'] font-semibold text-xl mb-4">Написать отзыв</h3>
                            
                            <div className="mb-4">
                                <label className="block text-gray-700 mb-2 text-sm">Оценка</label>
                            ...
                            <div className="mb-6">
                                <label className="block text-gray-700 mb-2 text-sm">Комментарий</label>
                                <textarea
                                    value={reviewComment}
            ...
            <AnimatePresence>
                {showEditModal && (
                    <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setShowEditModal(false)}>
                        <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
                            ...
                            <form onSubmit={handleUpdateProduct} className="p-6 space-y-4">
                                <div>
                                    <label className="block text-gray-700 mb-1 text-sm font-medium">Название *</label>
                                    <input
                                        type="text"
                                ...
                                <div>
                                    <label className="block text-gray-700 mb-1 text-sm font-medium">Категория *</label>
                                    <select
                                ...
                                <div>
                                    <label className="block text-gray-700 mb-1 text-sm font-medium">Цена *</label>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-gray-700 mb-1 text-sm font-medium">Техника вязки</label>
                                        <input
                                            type="text"
                                            value={editForm.technique}
                                            onChange={(e) => setEditForm(prev => ({ ...prev, technique: e.target.value }))}
                                            className="w-full p-3 rounded-xl bg-gray-100 outline-none focus:ring-2 focus:ring-firm-pink transition"
                                        />
                                    </div>
                                    <div>
                                        <label className="block text-gray-700 mb-1 text-sm font-medium">Размер</label>
                                        <input
                                            type="text"
                                            value={editForm.size}
                                ...
                                <div>
                                    <label className="block text-gray-700 mb-1 text-sm font-medium">Цвет</label>
                                    <input
                                        type="text"
                                ...
                                <div>
                                    <label className="block text-gray-700 mb-1 text-sm font-medium">Уход</label>
                                    <input
                                        type="text"
                                        value={editForm.care_instructions}
                                        onChange={(e) => setEditForm(prev => ({ ...prev, care_instructions: e.target.value }))}
                                        className="w-full p-3 rounded-xl bg-gray-100 outline-none focus:ring-2 focus:ring-firm-orange transition"
                                    />
                                </div>

                                <div>
                                    <label className="block text-gray-700 mb-1 text-sm font-medium">Описание</label>
                                    <textarea
                                ...
                                <div className="flex gap-3 pt-4">
                                    <button type="submit" disabled={editing} className="flex-1 py-3 bg-gradient-to-r from-firm-orange to-firm-pink text-white rounded-xl hover:shadow-lg transition disabled:opacity-50 font-medium">
                                        {editing ? 'Сохранение...' : 'Сохранить'}
                                    </button>
                                    <button type="button" onClick={() => setShowEditModal(false)} className="flex-1 py-3 border border-gray-300 rounded-xl hover:bg-gray-50 transition">
                                        Отмена
                                    </button>
                                </div>
                            </form>
