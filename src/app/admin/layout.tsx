...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
    ...
    // Обновите тип navigation:
        ...
        { name: 'Дашборд', href: '/admin/dashboard', icon: <DashboardIcon size={22} color="#737682" /> },
        { name: 'Пользователи', href: '/admin/users', icon: <UsersIcon size={22} color="#737682" /> },
        {
            name: 'Модерация', icon: <TagIcon size={22} color="#737682" />, children: [
                { name: 'Мастера', href: '/admin/moderation/masters', icon: <MasterIcon size={18} color="#737682" /> },
                { name: 'Товары', href: '/admin/moderation/products', icon: <ProductsIcon size={18} color="#737682" /> },
                { name: 'Блог', href: '/admin/moderation/blog', icon: <BlogIcon size={18} color="#737682" /> }
            ]
        },
        {
            name: 'Каталог', icon: <ProductsIcon size={22} color="#737682" />, children: [
                { name: 'Пряжа', href: '/admin/catalog/yarn' },
                { name: 'Категории товаров', href: '/admin/catalog/categories' }
            ]
        },
        {
            name: 'Поддержка', icon: <SupportIcon size={22} color="#737682" />, children: [
    ...
    return (
        <div className="min-h-screen bg-main">
            ...
            <AnimatePresence>
                {sidebarOpen && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-main-black/40 backdrop-blur-sm z-40 lg:hidden" onClick={() => setSidebarOpen(false)} />)}
            </AnimatePresence>
                                    ...
                                    {item.children ? (
                                        <div>
                                            <div className="flex items-center gap-3 px-4 py-3 text-text font-['Montserrat_Alternates'] font-medium rounded-lg">
                                                <span className="text-firm-gray">{item.icon}</span><span>{item.name}</span>
                                            </div>
                                            <ul className="ml-8 mt-1 space-y-1">
                                                {item.children.map((child) => (
                                                    <li key={child.name}>
                                                        <Link href={child.href} onClick={() => setSidebarOpen(false)} className={`block px-4 py-2 rounded-lg transition-all duration-300 font-['Montserrat_Alternates'] text-sm ${isActive(child.href) ? 'bg-linear-to-r from-firm-orange to-firm-pink text-main shadow-md' : 'text-firm-gray hover:bg-footer' }`}>
                                                            <div className="flex items-center gap-2">
                                                                {child.icon && <span className="opacity-60">{child.icon}</span>}{child.name}
                                                            </div>
                                                        </Link>
                                                    </li>
                                                ))}
                                        ...
                                        <Link href={item.href} onClick={() => setSidebarOpen(false)} className={`flex items-center gap-3 px-4 py-3 rounded-lg transition-all duration-300 font-['Montserrat_Alternates'] ${isActive(item.href) ? 'bg-linear-to-r from-firm-orange to-firm-pink text-main shadow-md' : 'text-text hover:bg-footer'}`}><span className={isActive(item.href) ? 'text-main' : 'text-firm-gray'}>{item.icon}</span><span>{item.name}</span></Link>
                                    ) : null}
                                </motion.li>
                            ))}
                        </ul>
