...
export async function POST(request: Request) {
    try {
        ...
        const { data: existingReview } = await supabase
            ...
            .eq('target_id', master_id)
            .eq('author_id', session.user.id)
            .maybeSingle();

        if (existingReview) {
            return NextResponse.json({ error: 'Вы уже оставляли отзыв этому мастеру' }, { status: 400 });
        }
        ...
        if (productIds.length === 0) {
            return NextResponse.json({ error: 'У мастера пока нет товаров' }, { status: 400 });
        }
        ...
        if (order_id) {
            ...
            // Проверяем любые завершенные заказы
        ...
        if (!hasValidOrder) {
            return NextResponse.json({ error: 'Вы можете оставить отзыв только после покупки' }, { status: 403 });
        }
        ...
        return NextResponse.json({ 
            ...
            review: {
                ...
                author_name: profile?.full_name || session.user.name || session.user.email?.split('@')[0] || 'Пользователь',
