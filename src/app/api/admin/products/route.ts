...
// POST - создание нового товара (опционально)
export async function POST(request: Request) {
    ...
    try {
        ...
        return NextResponse.json({ success: true, message: 'Товар создан' }, { status: 201 });
...
// DELETE - массовое удаление товаров (опционально)
// DELETE - массовое удаление товаров
export async function DELETE(request: Request) {
    ...
    try {
        ...
        if (productIds.length === 0) {
            return NextResponse.json({ error: 'ID товаров не указаны' }, { status: 400 });
        }
        ...
        if (invalidIds.length > 0) {
            return NextResponse.json({ 
                error: 'Неверный формат ID товаров',
                invalid_ids: invalidIds
            }, { status: 400 });
        }

        // Проверяем, существуют ли товары и принадлежат ли они активным мастерам
        const { data: existingProducts, error: fetchError } = await supabase
            .from('products')
            .select('id, title, status, master_id, main_image_url')
            .in('id', productIds);

        if (fetchError) {
            logError('Error fetching products for bulk delete', fetchError);
        ...
        if (orderError) {
            logError('Error checking order items', orderError);
            return NextResponse.json({ error: 'Ошибка проверки заказов' }, { status: 500 });
        }

        if (orderItems && orderItems.length > 0) {
            const usedProductIds = orderItems.map(item => item.product_id);
            return NextResponse.json({ 
                error: 'Некоторые товары находятся в заказах и не могут быть удалены',
        ...
        const deletedProducts: { id: string; title: string }[] = [];

        // Удаляем каждый товар
        for (const product of existingProducts) {
            try {
                // Удаляем изображения товара из S3
                if (product.main_image_url) {
                    ...
                    if (images && images.length > 0) {
                        for (const image of images) {
                            if (image.image_url) {
                                await deleteFromS3(image.image_url).catch(err => 
                                    logError(`Failed to delete image for product ${product.id}`, err, 'warning')
                                );
                            }
                        }
                    }
                    ...
                    await deleteFromS3(product.main_image_url).catch(err => 
                        logError(`Failed to delete main image for product ${product.id}`, err, 'warning')
                    );
                }

                // Удаляем товар (комментарии и избранное удалятся каскадно)
                ...
                // Логируем удаление каждого товара
                await supabase
                    .from('audit_logs')
                    .insert({
                        user_id: session.user.id,
                ...
                if (product.master_id) {
                    await supabase
                        ...
                        .insert({
                            ...
                            title: 'Товар удалён администратором',
                            message: `Ваш товар "${product.title}" был удалён из каталога.`,
                            type: 'product_deleted',
                            metadata: { product_id: product.id, bulk_delete: true },
                            created_at: now,
                            is_read: false
                        });
                }
                ...
                logError(`Error deleting product ${product.id}`, err, 'warning');
            }
        }
        ...
        logApiRequest('DELETE', '/api/admin/products', 200, Date.now() - startTime, session.user.id);
        logInfo(`Admin bulk deleted products`, { 
            adminId: session.user.id,
        ...
        const message = deletedCount > 0 
            ? `Успешно удалено: ${deletedCount} товаров${failedCount > 0 ? `, не удалось: ${failedCount}` : ''}`
        ...
        logError('Error bulk deleting products', error);
        return NextResponse.json({ error: 'Ошибка удаления товаров' }, { status: 500 });
    }
}