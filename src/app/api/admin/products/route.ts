...
export async function GET() {
    ...
    try {
        ...
        const result = await client.query(`
            ...
            WHERE p.status = 'moderation' OR p.status = 'draft'
...
export async function PUT(request: Request) {
    ...
    try {
        ...
        switch (action) {
            ...
            case 'reject':
                await client.query(
                    `UPDATE products SET status = 'rejected', updated_at = NOW(), moderation_comment = $2 WHERE id = $1`,
                    [productId, reason || 'Отклонено модератором']
                )
                break
            case 'draft':
                await client.query(
