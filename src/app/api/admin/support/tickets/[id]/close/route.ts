...
const closeTicketSchema = z.object({reason: z.string().max(500, 'Причина не может превышать 500 символов').optional(),});
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
    ...
    try {
        ...
        const validatedData = closeTicketSchema.parse({reason: body.reason});
        ...
        const { error: updateError } = await supabase.from('support_tickets').update({status: 'closed', closed_at: now,  closed_by: session.user.id, closed_reason: reason ? sanitize.text(reason) : null, updated_at: now}).eq('id', id);
        ...
        await supabase.from('audit_logs').insert({user_id: session.user.id, action: 'TICKET_CLOSED', entity_type: 'support_ticket', entity_id: id, new_values: { reason: reason || null }, created_at: now});

        const safeSubject = sanitize.text(existingTicket.subject || '');
        
        await supabase.from('notifications').insert({user_id: existingTicket.user_id, title: 'Обращение закрыто', message: reason ? `Ваше обращение "${safeSubject.substring(0, 50)}" закрыто. Причина: ${reason}` : `Ваше обращение "${safeSubject.substring(0, 50)}" закрыто. Спасибо, что обратились к нам!`, type: 'support', metadata: { ticket_id: id, reason: reason || null }, created_at: now, is_read: false});

        logApiRequest('POST', `/api/admin/support/tickets/${id}/close`, 200, Date.now() - startTime, session.user.id);
        logInfo(`Admin closed ticket`, { ticketId: id, adminId: session.user.id, reason: reason || null});

        return NextResponse.json({success: true, message: 'Тикет успешно закрыт', reason: reason || null}, { status: 200 });
