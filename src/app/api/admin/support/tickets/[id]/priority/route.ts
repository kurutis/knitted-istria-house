...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
    ...
    try {
        ...
        const validatedData = updatePrioritySchema.parse({priority: body.priority});

        const { priority } = validatedData;

        const { data: oldTicket, error: fetchError } = await supabase.from('support_tickets').select('id, priority, status, subject, user_id').eq('id', id).single();
        ...
        if (oldTicket.priority === priority) {return NextResponse.json({success: true, message: 'Приоритет уже установлен', priority: priority, priority_text: priorityText[priority]}, { status: 200 })}

        const now = new Date().toISOString();

        const { error: updateError } = await supabase.from('support_tickets').update({priority: priority, updated_at: now}).eq('id', id);
        ...
        await supabase.from('audit_logs').insert({user_id: session.user.id, action: 'TICKET_PRIORITY_CHANGED', entity_type: 'support_ticket', entity_id: id, old_values: { priority: oldTicket.priority }, new_values: { priority: priority }, created_at: now});

        const safeSubject = sanitize.text(oldTicket.subject || '');
        
        if (priority === 'high' && oldTicket.priority !== 'high') {
            await supabase.from('notifications').insert({user_id: oldTicket.user_id, title: 'Приоритет вашего обращения повышен', message: `Приоритет вашего обращения "${safeSubject.substring(0, 50)}" изменён на "Высокий". Мы уделим ему особое внимание.`, type: 'support', metadata: {ticket_id: id, priority: priority, old_priority: oldTicket.priority}, created_at: now, is_read: false});
        } else if (priority !== oldTicket.priority) {
            await supabase.from('notifications').insert({user_id: oldTicket.user_id, title: 'Приоритет обращения изменён', message: `Приоритет вашего обращения "${safeSubject.substring(0, 50)}" изменён с "${priorityText[oldTicket.priority]}" на "${priorityText[priority]}"`, type: 'support', metadata: {ticket_id: id, priority: priority, old_priority: oldTicket.priority}, created_at: now, is_read: false});
        }

        logApiRequest('PUT', `/api/admin/support/tickets/${id}/priority`, 200, Date.now() - startTime, session.user.id);

        return NextResponse.json({ success: true, message: `Приоритет тикета изменён на "${priorityText[priority]}"`, priority: priority,  priority_text: priorityText[priority]}, { status: 200 });
