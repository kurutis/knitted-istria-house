...
export async function GET(
    ...
    try {
        ...
        const result = await client.query(`
            ...
            WHERE m.chat_id = (SELECT chat_id FROM support_tickets WHERE id = $1)
...
export async function POST(
    ...
    try {
        ...
        `, [chatId, session.user.id, content.trim(), attachmentsJson]);
        ...
        const senderInfo = await client.query(`
            SELECT COALESCE(p.full_name, u.email) as sender_name, u.role
            FROM users u
            LEFT JOIN profiles p ON u.id = p.user_id
            WHERE u.id = $1
        `, [session.user.id]);
        ...
        return NextResponse.json({
            ...result.rows[0],
            attachments,
            sender_name: senderInfo.rows[0]?.sender_name || session.user.email,
            sender_role: 'admin'
        });
    } catch (error) {
