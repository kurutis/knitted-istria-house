...
const resendSmsSchema = z.object({email: z.string().email('Неверный формат email')});
...
export async function POST(request: Request) {
    ...
    try {
        ...
        const { data: user, error: findError } = await supabase.from('users').select('id, email, role, is_banned').eq('email', email).maybeSingle();
        ...
        const { data: profile, error: profileError } = await supabase.from('profiles').select('phone, phone_verified').eq('user_id', user.id).maybeSingle();
        ...
        if (!profile?.phone) {return NextResponse.json({ error: 'Номер телефона не указан в профиле' }, { status: 400 })}

        if (profile.phone_verified) {return NextResponse.json({ error: 'Телефон уже подтверждён' }, { status: 400 })}

        const smsCode = generateSMSCode();
        const expiresAt = new Date(Date.now() + 15 * 60 * 1000).toISOString();

        const { error: updateError } = await supabase.from('profiles').update({phone_verification_code: smsCode, phone_verification_expires: expiresAt, updated_at: new Date().toISOString()}).eq('user_id', user.id);
        ...
        logInfo('SMS resent successfully', {userId: user.id, email: email, phoneLast4: phone.slice(-4)});

        const response: ResendResponse = {success: true, message: 'Код подтверждения отправлен повторно', expiresIn: 900};

        if (process.env.NODE_ENV === 'development') response.testCode = smsCode
