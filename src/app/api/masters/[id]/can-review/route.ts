...
export async function GET(
    ...
    try {
        ...
        if (!session?.user) {
            return NextResponse.json({ canReview: false, error: 'Неавторизован' }, { status: 401 });
        }

        if (!id) {
            return NextResponse.json({ canReview: false, error: 'ID мастера обязателен' }, { status: 400 });
        }

        // Проверяем, не является ли пользователь самим мастером
        if (session.user.id === id) {
            return NextResponse.json({ canReview: false, reason: 'Нельзя оставить отзыв самому себе' });
        }
        ...
        if (existingReview) {
            return NextResponse.json({ canReview: false, reason: 'Вы уже оставляли отзыв этому мастеру' });
        }
        ...
        if (productIds.length === 0) {
            return NextResponse.json({ canReview: false, reason: 'У мастера пока нет товаров' });
        }

        // Получаем заказы пользователя, содержащие товары мастера
        const { data: orderItems, error } = await supabase
            ...
            .in('orders.status', ['delivered', 'completed']) as { data: OrderItemWithOrder[] | null; error: PostgrestError | null };
