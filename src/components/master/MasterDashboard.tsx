...
export default function MasterDashboard({
  ...
  const fetchMasterData = async () => {
    try {
      ...
      } else if (recentPostsData && typeof recentPostsData === 'object') {
        const possibleArrays = Object.values(recentPostsData).filter(Array.isArray);
        if (possibleArrays.length > 0) {
          recentPostsArray = possibleArrays[0];
        }
      }

      recentPostsArray = recentPostsArray.map(post => ({
        ...post,
      ...
      } else if (myPostsData && typeof myPostsData === 'object') {
        const possibleArrays = Object.values(myPostsData).filter(Array.isArray);
        if (possibleArrays.length > 0) {
          myPostsArray = possibleArrays[0];
        }
      }

      myPostsArray = myPostsArray.map(post => ({
        ...post,
  ...
  const enrichPostsWithAuthorData = async (
    ...
    try {
      ...
      return posts.map((post) => {
        ...
        return {
          ...post,
          author_name: profile?.full_name || post.author_name || "Мастер",
                              ...
                              {post.images.slice(0, 3).map((img, imgIdx) => {
                                ...
                                return imageUrl ? (
                                  <div key={imgIdx} className="relative aspect-video rounded-lg overflow-hidden bg-gray-100">
                                    <Image src={imageUrl} alt={`${post.title} - фото ${imgIdx + 1}`} fill className="object-cover hover:scale-105 transition-transform duration-300" />
                                  </div>
                                ) : null;
                              })}
                ...
                {myPosts.length === 0 ? (
                  ...
                  myPosts.map((post, idx) => (
                    <motion.div
                      ...
                      className="p-6 transition-all duration-300"
                    >
                      {/* Аналогичная структура как для recentPosts */}
                      <div className="max-w-3xl mx-auto">
                        ...
                        {/* Комментарии - аналогично как в recentPosts */}
                        <AnimatePresence>
                          {showComments === post.id && (
                            <motion.div
                              initial={{ opacity: 0, height: 0 }}
