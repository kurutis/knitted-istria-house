...
import MasterProductsList from "@/components/master/MasterProductsList"
    ...
    const fetchMasterData = async () => {
            try{
                ...
                const monthlyOrders = Array.isArray(orders)
                    ? orders.filter((o: any) => new Date(o.created_at) > thirtyDaysAgo)
                    : []
                            ...
                            <div className="space-y-6">
                                <div className="flex gap-4 justify-between">
                                    <div className="bg-white rounded-lg shadow-md p-6 w-full">
                                        ...
                                        <p className="text-xs text-firm-green mt-1">+{stats.monthly_views} за месяц</p>
                                    </div>
                                    <div className="bg-white rounded-lg shadow-md p-6 w-full">
                                        ...
                                        <p className="text-xs text-firm-green mt-1">+{stats.monthly_orders} за месяц</p>
                                    </div>
                                    <div className="bg-white rounded-lg shadow-md p-6 w-full">
                                        <p className="text-gray-500 text-sm font-['Montserrat_Alternates']">Выручка</p>
                                        <p className="text-3xl font-bold font-['Montserrat_Alternates'] text-firm-green">{stats.total_revenue.toLocaleString()} ₽</p>
                                        <p className="text-xs text-firm-green mt-1">+{stats.monthly_revenue.toLocaleString()} ₽ за месяц</p>
                                    </div>
                                    <div className="bg-white rounded-lg shadow-md p-6 w-full">
                                        <p className="text-gray-500 text-sm font-['Montserrat_Alternates']">Подписчики</p>
                                        <p className="text-3xl font-bold font-['Montserrat_Alternates'] text-firm-orange">{stats.total_followers}</p>
                                ...
                                <div className="bg-white rounded-lg shadow-md p-6">
                                    ...
                                    <div className="flex justify-between gap-5">
                                        <Link href="/master/products/new" className="w-full px-4 py-2 bg-firm-orange text-white rounded-lg hover:bg-opacity-90 transition-all duration-300 text-center">+ Добавить товар</Link>
                                        <Link href="/master/blog/new" className="w-full px-4 py-2 bg-firm-pink text-white rounded-lg hover:bg-opacity-90 transition-all duration-300 text-center">+ Новая запись в блоге</Link>
                                        <Link href="/master/master-classes/new" className="w-full px-4 py-2 border-2 border-firm-orange text-firm-orange rounded-lg hover:bg-firm-orange hover:text-white transition-all duration-300 text-center">+ Создать мастер-класс</Link>
                                    </div>
                                </div>
                                <div className="bg-white rounded-lg shadow-md p-6">
                                            ...
                                            .map((product: any) => (
                                                <div key={product.id} className="flex justify-between items-center p-4 border rounded-lg hover:shadow-md transition-shadow">
                                                    <div>
                                                        ...
                                                        <p className="text-sm text-gray-500 mt-1">{product.views} просмотров</p>
                                                    </div>
                        ...
                        {activeTab === 'blog' && (
                            <div className="bg-white rounded-lg shadow-md p-6">
                                <div className="flex justify-between items-center mb-6">
                                    ...
                                    <Link href="/master/blog/new" className="px-4 py-2 bg-firm-pink text-white rounded-lg hover:bg-opacity-90 transition-all duration-300 font-['Montserrat_Alternates'] flex items-center gap-2">+ Новая запись</Link>
                                </div>
                                <div className="space-y-4">
                                    {blogPosts.map((post: any) => (
                                        <div key={post.id} className="border rounded-lg p-5 hover:shadow-md transition-shadow">
                                            ...
                                            <p className="text-gray-600 mb-3 line-clamp-2">{post.excerpt || post.content?.substring(0, 200)}...</p>
                                            <div className="flex justify-between items-center">
                        ...
                        {activeTab === 'master-classes' && (
                            <div className="bg-white rounded-lg shadow-md p-6">
                                <div className="flex justify-between items-center mb-6">
                                    ...
                                    <Link href="/master/master-classes/new" className="px-4 py-2 bg-firm-orange text-white rounded-lg hover:bg-opacity-90 transition-all duration-300 font-['Montserrat_Alternates'] flex items-center gap-2">+ Создать мастер-класс</Link>
                                </div>

                                <div className="flex gap-2 mb-6 overflow-x-auto pb-2">
                                    <button onClick={() => setMasterClassFilter('all')} className={`px-4 py-2 rounded-lg text-sm whitespace-nowrap transition ${masterClassFilter === 'all' ? 'bg-firm-orange text-white' : 'border border-gray-300 hover:bg-gray-50'}`}>Все ({masterClasses.length})</button>
                                    <button onClick={() => setMasterClassFilter('published')} className={`px-4 py-2 rounded-lg text-sm whitespace-nowrap transition ${masterClassFilter === 'published' ? 'bg-firm-orange text-white' : 'border border-gray-300 hover:bg-gray-50'}`}>Опубликованные ({masterClasses.filter((mc: any) => mc.status === 'published').length})</button>
                                    <button onClick={() => setMasterClassFilter('draft')} className={`px-4 py-2 rounded-lg text-sm whitespace-nowrap transition ${masterClassFilter === 'draft' ? 'bg-firm-orange text-white' : 'border border-gray-300 hover:bg-gray-50'}`}>Черновики ({masterClasses.filter((mc: any) => mc.status === 'draft').length})</button>
                                    <button onClick={() => setMasterClassFilter('cancelled')} className={`px-4 py-2 rounded-lg text-sm whitespace-nowrap transition ${masterClassFilter === 'cancelled' ? 'bg-firm-orange text-white' : 'border border-gray-300 hover:bg-gray-50'}`}>Отмененные ({masterClasses.filter((mc: any) => mc.status === 'cancelled').length})</button>
                                </div>

                                {masterClasses.length === 0 ? (
                                    <div className="text-center py-12 bg-gray-50 rounded-lg">
                                        <p className="text-gray-500 mb-4">У вас нет созданных мастер-классов</p>
                                        <Link href="/master/master-classes/new" className="inline-block px-6 py-3 bg-firm-orange text-white rounded-lg hover:bg-opacity-90 transition">Создать первый мастер-класс →</Link>
                                    </div>
                                ) : (
                                    <div className="space-y-4">
                                        {masterClasses.filter((mc: any) => masterClassFilter === 'all' || mc.status === masterClassFilter).map((mc: any) => (
                                                ...
                                                <div className="flex gap-4">
                                                    {mc.image_url && (
                                                        <div className="w-32 h-32 shrink-0 rounded-lg overflow-hidden bg-gray-100">
                                                            <Image src={mc.image_url} alt={mc.title} className="w-full h-full object-cover" width={160} height={160} />
                                                        </div>
                                                    )}
                                                    <div className="flex-1">
                                                        <div className="flex justify-between items-start">
                                                            <div>
                                                                ...
                                                                <div className="flex items-center gap-2 mt-1">
                                                                    ...
                                                                    <span className={`px-2 py-0.5 rounded-full text-xs ${getStatusColor(mc.status)}`}>{getStatusText(mc.status)}</span>
                                                                </div>
                                                            </div>
                                                        ...
                                                        <div className="flex flex-wrap gap-3 mt-2 text-xs text-gray-500">
                                                            ...
                                                            {mc.type === 'offline' && mc.location && (<div className="flex items-center gap-1">📍 {mc.location}</div>)}
                                                        </div>
                                                        <div className="mt-3 flex justify-end gap-2">
                                                            <button onClick={() => { setSelectedMasterClass(mc); setShowParticipantsModal(true) }} className="px-3 py-1.5 bg-gray-100 text-gray-700 rounded-lg text-sm hover:bg-gray-200 transition">👥 Участники ({mc.registrations?.length || 0})</button>
                                                            {mc.status === 'published' && new Date(mc.date_time) > new Date() && (
                                                                <button onClick={() => handleCancelMasterClass(mc.id)} className="px-3 py-1.5 border border-red-500 text-red-500 rounded-lg text-sm hover:bg-red-500 hover:text-white transition">Отменить</button>
                                                            )}
                                                            {mc.status === 'draft' && (
                                                                <>
                                                                    <Link href={`/master/master-classes/${mc.id}/edit`} className="px-3 py-1.5 bg-firm-orange text-white rounded-lg text-sm hover:bg-opacity-90 transition">Редактировать</Link>
                                                                    <button onClick={() => handleDeleteMasterClass(mc.id)} className="px-3 py-1.5 bg-red-500 text-white rounded-lg text-sm hover:bg-red-600 transition">Удалить</button>
                                                                </>
                                                            )}
                                                        </div>
                                                    </div>
                            ...
                            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" onClick={() => setShowParticipantsModal(false)}>
                                <div className="bg-white rounded-xl max-w-lg w-full max-h-[80vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
                                    <div className="sticky top-0 bg-white border-b border-gray-200 p-4 flex justify-between items-center">
                                        <h2 className="font-['Montserrat_Alternates'] font-semibold text-xl">Участники: {selectedMasterClass.title}</h2>
                                        <button onClick={() => setShowParticipantsModal(false)} className="text-gray-500 hover:text-gray-700 text-xl">✕</button>
                                    </div>
                        ...
                        {activeTab === 'profile' && (
                            <div className="bg-white rounded-lg shadow-md p-6">
                                <div className="flex justify-between items-center mb-6">
                                    ...
                                    {!isEditing ? (
                                        <button className="px-4 py-2 border-2 border-firm-pink rounded-lg hover:scale-105 hover:border-4 hover:bg-firm-pink hover:text-white transition-all duration-300 font-['Montserrat_Alternates']" onClick={() => setIsEditing(true)}>Редактировать</button>
                                    ) : (
                                        <button className="px-4 py-2 bg-gray-500 text-white rounded-lg hover:bg-gray-600 transition-all duration-300 font-['Montserrat_Alternates']" onClick={() => { setIsEditing(false); setAvatarFile(null); setAvatarPreview(null); }}>Отмена</button>
                                    )}
                                </div>
                                {isEditing ? (
                                    <form onSubmit={handleProfileUpdate} className="space-y-4">
                                        ...
                                        <div>
                                            <label className="block text-gray-700 mb-1 font-['Montserrat_Alternates']">Адрес</label>
                                            <input className="w-full p-2 rounded-lg bg-[#eaeaea] outline-firm-pink focus:outline-2" type="text" name="address" value={profileData.address || ''} onChange={handleInputChange} placeholder="ул. Примерная, д. 1"/>
                                        </div>
                                        <div>
                                        ...
                                        <div className="flex items-center gap-3">
                                            <div className="relative flex items-center">
                                                ...
                                                {profileData.custom_orders_enabled && (
                                                    <svg className="absolute w-4 h-4 text-white left-0.5 top-0.5 pointer-events-none" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><polyline points="20 6 9 17 4 12"></polyline></svg>
                                                )}
                                            </div>
                                            <label className="text-gray-700 cursor-pointer select-none font-['Montserrat_Alternates']">Принимаю индивидуальные заказы</label>
                                        </div>
                                        <button type="submit" disabled={saving} className="w-full mt-6 p-3 bg-firm-pink text-white rounded-lg hover:scale-105 transition-all duration-300 font-['Montserrat_Alternates'] font-semibold disabled:opacity-50 disabled:cursor-not-allowed">
                                    ...
                                    <div className="space-y-4">
                                        <div className="grid grid-cols-2 gap-4">
                                            ...
                                            <div className="col-span-2">
                                                ...
                                                <p className="text-lg font-medium">{profileData.custom_orders_enabled ? '✅ Принимаю' : '❌ Не принимаю'}</p>
                                            </div>
                                        </div>
                                    </div>
                                )}
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="border-t border-gray-400 pt-6">
                                        ...
                                        <div className="space-y-2">
                                            <label className="flex items-center gap-3"><input type="checkbox" className="w-5 h-5 accent-firm-orange" defaultChecked /><span className="font-['Montserrat_Alternates']">О новых заказах</span></label>
                                            <label className="flex items-center gap-3"><input type="checkbox" className="w-5 h-5 accent-firm-pink" defaultChecked /><span className="font-['Montserrat_Alternates']">О сообщениях от покупателей</span></label>
                                            <label className="flex items-center gap-3"><input type="checkbox" className="w-5 h-5 accent-firm-orange" /><span className="font-['Montserrat_Alternates']">О новых отзывах</span></label>
                                        </div>
                                    </div>
                                    <div className="border-t border-gray-400 pt-6">
                                        <h3 className="font-['Montserrat_Alternates'] font-semibold text-lg mb-3">Магазин</h3>
                                        <div className="space-y-2">
                                            <label className="flex items-center gap-3"><input type="checkbox" className="w-5 h-5 accent-firm-pink" defaultChecked /><span className="font-['Montserrat_Alternates']">Автоматически подтверждать заказы</span></label>
                                            <label className="flex items-center gap-3"><input type="checkbox" className="w-5 h-5 accent-firm-orange" /><span className="font-['Montserrat_Alternates']">Отображать мои товары в поиске</span></label>
                                        </div>
                                    </div>
                                    <div className="border-t border-gray-400 pt-6">
