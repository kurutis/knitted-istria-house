        ...
        <Link href={`/catalog/${product.id}`} className="group">
            <div className="bg-main rounded-xl overflow-hidden hover:shadow-lg transition-all duration-300">
                <div className="relative aspect-square bg-main">
                    ...
                    <button onClick={handleAddToFavorite}  disabled={loading} onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)} className="absolute top-2 right-2 w-8 h-8 rounded-full flex items-center justify-center transition-all duration-300 z-10 disabled:opacity-50"><FavoriteIcon className={`w-8 h-8 transition-all duration-300 ${isHovered && !isFavorite ? 'scale-110 rotate-12' : '' } ${isFavorite ? 'scale-110' : ''}`} isFavorite={isFavorite} /></button>
                </div>
